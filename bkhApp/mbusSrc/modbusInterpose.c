 ...
 * file implements the interpose interface that adds the required header
 * information for TCP, RTU serial or ASCII serial before sending the frame to
 * the underlying driver.  It removes header information from the response
 * frame before returning it to the Modbus driver.
 *
 * Author: Mark Rivers
 */
