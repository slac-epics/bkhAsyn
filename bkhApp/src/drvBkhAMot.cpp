 ...
 *---------------------------------------------------------------------------*/
  new drvBkhAMot(port, modbusPort, motorE, addr, func, len, nchan, tmof, tmos);
...
/*-----------------------------------------------------------------------------
 * Driver setup.  This must be called after drvBkhAMotConfig().
 * home: 1 at negative limit, 2 at current position, 3 at positive limit.
 * nlim: 1 negative limit switch is present, 0 not present.
 * plim: 1 positive limit switch is present, 0 not present.
 *---------------------------------------------------------------------------*/
 drvBkhAMot *pmotor = (drvBkhAMot*)findAsynPortDriver(port);
  ...
  pmotor->motorSetup(port, home, nlim, plim);
}
